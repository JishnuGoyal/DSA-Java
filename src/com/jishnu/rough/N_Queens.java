...
public class N_Queens {
    ...
    private static boolean isSafe(boolean[][] board, int r, int c) {
        ...
        // we only need to check till the current value of row, as the function frames have not picked up the seats which are ahead
        for (int i = 0; i < r /*board.length*/; i++) { // traverses the rows
            if (board[i][c]) return false;
        }

        // same logic
        for (int j = 0; j < c /*board[0].length*/; j++) { // traverses the columns
